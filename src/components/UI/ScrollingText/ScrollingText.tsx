...
class ScrollingText extends Component<any, any> {
  ...
  componentDidUpdate(prevProps: any, prevState: any) {
    this.timeout = setTimeout(() => {
      let newVal =
        this.state.activeTextId < this.props.items.length - 1
          ? this.state.activeTextId + 1
          : (this.state.activeTextId + 1) % this.props.items.length
    ...
    return (
      <span className="ScrollingText">
        {this.props.items.map((item: any) => (
          <span
            key={item.id}
            style={{ visibility: this.getVisibility(item.id) }}
          >
            {item.label}
          </span>
        ))}
